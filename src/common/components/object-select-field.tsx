...
export interface ObjectSelectFieldProperties<T>
    extends Formik.FieldProps<T>,
...
export class ObjectSelectField<T> extends React.Component<
    ObjectSelectFieldProperties<T>
> {
    ...
    public componentDidUpdate(
        prevProps: Readonly<ObjectSelectFieldProperties<T>>
    ) {
        // Reload value map
        this.valuesByKeyLookup.clear()
        this.props.values.forEach((v, i) =>
            this.valuesByKeyLookup.set(this.props.keySelector(v), v)
        )
                ...
                >
                    {
                        !currentKey && (
                            <option></option>
                        ) /* blank option if value doesn't exist in options */
                    }
                    {this.props.values.map((v, i) => (
                        <option key={i} value={this.props.keySelector(v)}>
                            {this.props.labelSelector(v)}
                        </option>
                    ))}
                </select>
            </div>
