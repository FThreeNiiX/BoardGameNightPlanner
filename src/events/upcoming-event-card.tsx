...
export const UpcomingEventCard: React.FC<UpcomingEventCardProperties> = (
    ...
    const otherAttendees = keys
        .filter((k) => k !== props.user.id)
        .map((k) => props.event.data.attendees[k])
        .sort((a1, a2) => {
    ...
    if (attending) {
        status = (
            <em>
                <i className="fas fa-check" /> You're Attending!
            </em>
        )
    }
        ...
        actions = (
            <div className="card-footer">
                <button
                    ...
                    onClick={async () =>
                        await attendEvent(props.event.id, props.user)
                    }
                >
                    Attend
                </button>
        ...
        actions = (
            <div className="card-footer">
                <button
                    ...
                    className="card-footer-item"
                    onClick={async () =>
                        await unattendEvent(props.event.id, props.user.id)
                    }
                >
                    Unattend
                </button>
